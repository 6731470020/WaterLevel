...
- ต้องมีจุดเทียบค่าอย่างน้อย **2 จุด** มิฉะนั้นโยน `ValidationError`
  **ห้ามมีค่าสำรองฮาร์ดโค้ด** แบบระบบเดิม (`api/broadcast-line.php:411`) เพราะทำให้รายงานผิดโดยไม่มีสัญญาณเตือน
- เรียงจุดตามพิกเซลจากน้อยไปมาก
- อยู่ในช่วง → ประมาณค่าเชิงเส้น
- นอกช่วง → คำนวณต่อด้วยความชันของคู่ปลายที่ใกล้ที่สุด
- คืนค่าทศนิยม 2 ตำแหน่ง และต้องมีเมท็อดผกผัน `meterToPixel()` สำหรับวาดเส้นบนภาพ

ข้อมูลจริงของจุดวัดที่ 1: `[204→4.00, 247→3.80, 286→3.60, 309→3.50, 327→3.40, 368→3.20]`
ตัวอย่าง: พิกเซล 298 อยู่ระหว่าง (286, 3.60) กับ (309, 3.50) → `3.60 − (12÷23 × 0.10) = 3.55 ม.`
